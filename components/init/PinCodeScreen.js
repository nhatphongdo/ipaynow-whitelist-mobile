...
class PinCodeScreen extends React.Component {
  constructor(props) {
    super(props);
    const type = props.navigation.getParam("type", EnterPinCode);
    ...
    this.state = {
      ...
      lockedTimer: null,
      lockedTimeText: "",
      mustVerifyPinCode: props.navigation.getParam("mustVerifyPinCode", false)
    };
  ...
  handleEnterSuccess = () => {
    const handler = this.props.navigation.getParam("onEnterSuccess", null);
  ...
  handleChooseSuccess = async () => {
    ...
    const handler = this.props.navigation.getParam("onChooseSuccess", null);
    this.props.navigation.goBack();
    if (handler && typeof handler === "function") {
      ...
      >
        {_codes.map((val, index) =>
          this.state.pinCode.length > index ? (
            <SelectedPinDot key={index} style={styles.dot} fill="#186BFE" />
          ) : (
