...
class ReceiveScreen extends React.Component {
  ...
  onAmountChanged = async (value) => {
    ...
    if (this.state.currency !== USDT) {
      amountValue = numeral(await this.props.convertRate(this.state.currency, USDT, amountValue.value()))
    }
  ...
  onCurrencyChanged = async () => {
    ...
    if (isNaN(amountValue.value()) || amountValue.value() === null) {
      ...
      if (this.state.currency === USDT) {
        amount = formatCurrency(await this.props.convertRate(USDT, this.props.settings.currency, amountValue.value()))
      } else {
        amount = formatCrypto(await this.props.convertRate(this.state.currency, USDT, amountValue.value()))
      }
    }
    ...
    if (this.state.currency === USDT) {
      ...
      this.setState({ currency: USDT }, () => this.onAmountChanged(amount))
    }
  }
