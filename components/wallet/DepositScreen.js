...
import { ETH, HDN } from "../../stores/rates/constants";
...
class DepositScreen extends React.Component {
  ...
  onAmountChanged = async text => {
    ...
    if (!isNaN(amountValue.value()) && amountValue.value() !== null && amountValue.value() > 0) {
      const eth = await this.props.convertRate(HDN, ETH, amountValue.value());
  ...
  send = async (address, ethAmount) => {
    ...
    await this.props.updateTransaction(
      ...
      ethAmount.value(),
      "ETH",
      `${translate("Buy")} ${formatCrypto(this.state.amount)} ${translate("HDN")}`,
      DEPOSIT,
              ...
              {false && (
                <View smallSpaceTop style={styles.row}>
                  <Button
                    iconLeft
                    toggle
                    active={this.state.depositWallet === HdnWallet}
                    tinySpaceRight
                    flexFull
                    onPress={() => this.setState({ depositWallet: HdnWallet })}
                  >
                    <RubyIcon
                      fill={
                        this.state.depositWallet === HdnWallet
                          ? ThemeService.getThemeStyle().variables.buttonToggleOnColor
                          : ThemeService.getThemeStyle().variables.buttonToggleOffColor
                      }
                    />
                    <StyledText>{translate("HDN")}</StyledText>
                  </Button>
                  <Button
                    iconLeft
                    toggle
