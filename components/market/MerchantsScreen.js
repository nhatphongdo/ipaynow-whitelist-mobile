    ...
    if (status !== "granted") {
      ...
      if (status === "granted") {
        ...
        this.setState(
          {
            cca2: "US",
            country: countryData.name[AVAILABLE_LANGUAGES[this.props.settings.language].countryPicker]
          },
          this.loadMerchants
        );
        return;
      ...
      <Screen title={translate("MERCHANTS")}>
        <Container style={styles.container}>
          <Content style={styles.container} contentContainerStyle={styles.contentContainer}>
            ...
            <GroupBox smallSpaceTop icon={<MerchantsIcon {...ThemeService.getThemeStyle().groupBoxHeaderIcon} />} title={translate("MERCHANTS")} fullHeight>
              <View row flexCenter>
                <Search
                  placeholder={translate("Search") + " " + this.state.country}
                  onTextChanged={text => this.setState({ search: text }, this.loadMerchants)}
                />
                <Button smallSpaceLeft shadow style={styles.countryPicker} onPress={() => this._countryPicker.openModal()}>
                  <CountryPicker
